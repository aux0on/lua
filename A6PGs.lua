...
local __Debris = getfserv("Debris")
...
local __NORMAL_COLOR = ColorSequence.new({
    ColorSequenceKeypoint.new(0, __RGB(0, 85, 255)),
    ColorSequenceKeypoint.new(1, __PCLR(0, 0, 0))
})
...
local function ApplyCustomStyle(imageButton, buttonMaid)
    -- UIStroke on the ImageButton itself
    ...
    -- Spin the gradient rotation each frame (mirrors original behaviour)
    buttonMaid:GiveTask(__RS.RenderStepped:Connect(function()
        gradient.Rotation = (gradient.Rotation + 1) % 360
    end))

    -- Click SFX (fires on every click, independent of toggle debounce)
    buttonMaid:GiveTask(imageButton.MouseButton1Click:Connect(function()
        local sfx = Instance.new("Sound", imageButton)
        sfx.Name = "reina ins't gay :|||"
        sfx.SoundId = "rbxassetid://12221967"
        sfx.Volume = 1
        sfx:Play()
        __Debris:AddItem(sfx, 1)
    end))

    return gradient  -- returned so onClick can swap .Color for toggled state
end

local function MakeDraggable(gui, maid, ripple, clickFunc)
...
function BindableButtons.AddBButton(id, text, onFunc, offFunc)
    ...
    ImageButton.BackgroundTransparency = 0.3  -- from ApplyCustomStyle
    ...
    TextLabel.Name = "@Text"
    TextLabel.Size, TextLabel.Position, TextLabel.AnchorPoint = __UD2(0.8, 0, 0.8, 0), __UD2(0.5, 0, 0.5, 0), __V2(0.5, 0.5)
    TextLabel.BackgroundTransparency = 1
    TextLabel.Font = Enum.Font.SourceSansLight  -- from ApplyCustomStyle
    TextLabel.Text = text
    TextLabel.TextColor3, TextLabel.TextSize, TextLabel.TextWrapped = __PCLR(1, 1, 1), 10, true
    TextLabel.ZIndex = 3

    Instance.new("UIAspectRatioConstraint", ImageButton).AspectRatio = 1

    -- Apply the custom style; get back the gradient so toggle can recolor it
    local strokeGradient = ApplyCustomStyle(ImageButton, buttonMaid)
    ...
    local function onClick()
        ...
        strokeGradient.Color = BindValue.Value and __TOGGLED_COLOR or __NORMAL_COLOR
    ...
    MakeDraggable(ImageButton, buttonMaid, ripple, onClick)
...
function BindableButtons.DeleteBButton(id)
    if BindableButtons.Maids[id] then
        ...
        BindableButtons.Maids[id], BindableButtons.Buttons[id] = nil, nil
    end
end

-- Creates a button
-- Arg1: ID  |  Arg2: Text  |  Arg3: Callback (ON)  |  Arg4: Callback (OFF)
local flyBind = BindableButtons.AddBButton(
    "Test",
    "test",
...
flyBind.Changed:Connect(function(val)
    print("State is now:", val)
end)
